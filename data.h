...
void getMetadata(AVFormatContext* handle, char** title, char** album, char** artist, int* trackNo) {
    ...
    *artist = NULL;
    ...
    AVDictionaryEntry *ent = av_dict_get(dict, "artist", NULL, AV_DICT_IGNORE_SUFFIX);
    if(ent != NULL) {
        *artist = strdup(ent->value);
    }

    ent = av_dict_get(dict, "album", NULL, AV_DICT_IGNORE_SUFFIX);
    if(ent != NULL) {
        *album = strdup(ent->value);
    }

    ent = av_dict_get(dict, "title", NULL, AV_DICT_IGNORE_SUFFIX);
    if(ent != NULL) {
